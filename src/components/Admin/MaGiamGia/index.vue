                           ...
                           <tr>
                              ...
                              <td class="align-middle text-center">
                                 <button v-on:click="Object.assign(edit_ma_giam_gia, value)"
                                    class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#capnhatDM">Cập
                                    nhật</button>
...
export default {
   ...
   methods: {
      TimKiemBE() {
         axios
            .post('http://127.0.0.1:8000/api/admin/ma-giam-gia/tim-kiem', this.search)
            .then((res) => {
      ...
      changeTrangThai(value) {
         axios
            .post("http://127.0.0.1:8000/api/admin/ma-giam-gia/change-status", value)
            .then((res) => {
               if (res.data.status) {
                  var thong_bao = '<b>Thông báo</b><span style="margin-top: 5px">' + res.data.message + '<span>';
                  this.$toast.success(thong_bao);
